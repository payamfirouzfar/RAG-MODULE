...
| A67 | An `ExecutionPlan` can be run via `ExecutionEngine.execute_plan(plan, handler, *, context=None, executor=None)` under the same `Run`/`Trace`/`MetricsCollector` lifecycle guarantee `execute(module, ...)` already provides for a single `Module` call — exactly one `Run`, at most one span (at `DEBUG`), one plan-level duration metric — with plan iteration fully delegated to a substitutable `Executor` (defaulting to `SequentialExecutor()`), without the engine deriving plan order, resolving `ExecutionStep -> Component` itself, injecting a per-step `ExecutionContext` into `StepHandler` (whose ADR-018 signature has no slot for one), or changing `execute(module, ...)` in any way | ADR-019 (Proposed — not yet implemented); grounded in a 15A/15B repository audit of `engine.py`/`run.py`/`trace.py`/`metrics.py`/`context.py`/`sequential.py`/`test_nested_execution.py` (full files, not assumed), which found that `execute(module, ...)` never wraps exceptions itself (bare `raise` after `run.fail(exc)`) and that ADR-007 already made "no per-child nested spans" a tested non-goal (`test_engine_full_rag_shaped_pipeline`: `len(result.trace.spans) == 1` for a 3-stage nested pipeline) — both precedents this ADR deliberately mirrors rather than reopens; also found and explicitly named (not silently inherited or silently fixed) that `execute(module, ...)` has no existing protection against observability itself raising (ADR-019 Q15) | implementation, tests, benchmark, and CI all pending — this row records the architectural decision only, per the project's standing rule that an ADR decision is not proof until implementation/tests/CI independently confirm it | planned: PLAN/CTX/TRACE/METRIC/FAIL/COMPAT/OBS/SEC/API/STATE test matrix per ADR-019's Testing strategy section, `benchmarks/step15_plan_execution_runtime.py` (10/100/1,000 steps, isolating engine overhead from ADR-018's already-benchmarked snapshot-copying cost) |
