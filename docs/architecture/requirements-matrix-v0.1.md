...
| A68 | A `CompositionGraph` can be wrapped as a reusable `Block` — a `Module` subclass structurally satisfying `Component` with zero special case — with an explicit, required `input_node`/`output_node` pair (no first/last-node inference, no multi-root/multi-sink support in this version), delegating all execution to the existing, unmodified `plan()`/`Executor`/`ExecutionEngine.execute_plan` chain (no second execution system, no plan caching, no new exception type), such that a `Block` can itself be nested as a `GraphNode.component` inside an outer `CompositionGraph`, or as a child of a `Sequential`, with zero runtime-layer changes required for either — fulfilling A2 ("Blocks compose Components... Block abstraction not yet formalized," open since Step 1) now that every layer ADR-016 deferred it pending (`CompositionGraph`/`ExecutionPlan`/`Executor`/`ExecutionEngine.execute_plan`) is `Accepted` | ADR-020 (Proposed — not yet implemented); grounded in a 16A/16B/16C repository audit of `sequential.py`/`module.py`/`component.py`/`composition.py`/`connection.py`/`ports.py`/`execution_plan.py`/`engine.py` (full files, not assumed), which found (1) `GraphNode.component` is accepted structurally-typed and unchecked — confirmed via grep, zero `isinstance(..., Component)` calls exist anywhere in the composition/execution stack, proving the runtime is already Block-agnostic by construction; (2) no entry/exit-node concept exists anywhere in `CompositionGraph`/`ExecutionPlan`, forcing `input_node`/`output_node` to be explicit rather than inferred; (3) `Module.__init__` takes no arguments anywhere in this codebase, so `Block` gets no `name=` override, matching every existing subclass exactly; `src/ragtorch/core/block.py` — `Block(Module)`, `Block.forward` builds a `StepHandler` closure resolving each node's `GraphNode.component` and calling `ExecutionEngine().execute_plan(plan(self._graph), handler, context=context)`, returning `result.output.values[self._output_node]`; pseudocode verified end-to-end against real repository code before being frozen into the ADR (`2 → AddOne → 3 → MultiplyTwo → 6`; `isinstance(block, Component) is True`; a component failure propagates as `ExecutionError` via `Module.__call__`'s existing, unmodified wrapping with the original exception as `__cause__`) | implementation, tests, benchmark, and CI all pending — this row records the architectural decision only, per the project's standing rule that an ADR decision is not proof until implementation/tests/CI independently confirm it | planned: CONSTRUCT/CONTRACT/EXEC/NEST/FAIL/COMPAT/SEC/API test matrix per ADR-020's Testing strategy section, `benchmarks/step16_block_composition.py` (3/10/30-node chains, measuring per-call `plan()` re-derivation overhead as the explicit evidence gate for ADR-020 Q11's deferred plan-caching decision) |
