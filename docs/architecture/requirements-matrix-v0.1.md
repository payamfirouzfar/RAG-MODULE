...
| A69 | A `CompositionGraph` can be turned into a complete, graph-backed architecture via `RAGModule.from_graph(graph, *, input_node, output_node)`, returning an actual `RAGModule` instance (`isinstance(x, RAGModule)` and `isinstance(x, Module)` both `True`) that delegates all execution to the existing, unmodified `Block`/`plan()`/`Executor`/`ExecutionEngine.execute_plan` chain, without changing `RAGModule`'s existing class hierarchy, constructor, or subclass-and-override-`forward()` pattern, without introducing a second execution system, plan caching, serialization, async execution, or a new exception type, and without supporting `from_graph` on arbitrary `RAGModule` subclasses in this version | ADR-021 (Proposed — not yet implemented); grounded in a 17A/17B/17C repository audit of `module.py`/`block.py`/`sequential.py`/`test_module.py` (full files, not assumed), which found and corrected two issues before any code was committed: (1) a private adapter that both calls `register_module` explicitly and assigns the same value to an attribute double-registers the child (`Module.__setattr__` auto-registers any `Module`-valued attribute) — caught via direct construction and `.inspect()` inspection, fixed to a single `self._block = block` assignment; (2) an initial draft assumed single-hop `exc.__cause__ is original_error` failure-propagation identity, disproven by direct construction against already-merged `main` behavior — `Block` nested in `Sequential` already produces a two-layer `ExecutionError` chain today, since `Module.__call__` wraps at every layer it passes through; corrected to the accurate, walked-`__cause__`-chain contract (ADR-021 Q8) | implementation, tests, benchmark, and CI all pending — this row records the architectural decision only, per the project's standing rule that an ADR decision is not proof until implementation/tests/CI independently confirm it | planned: RM/COMPAT test matrix per ADR-021's Testing strategy section, `benchmarks/step17_ragmodule_architecture.py` (3/10/30-node chains, measuring the additional `Module.__call__` layer's overhead over calling `Block` directly) |
