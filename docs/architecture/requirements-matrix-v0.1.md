...
| A66 | An `ExecutionPlan` can be run to completion via a caller-supplied `StepHandler` (keyed only by `node_id`), producing an `ExecutionResult` keyed by `node_id`, with each step's handler able to see prior steps' results through an immutable `ExecutionContext`, stopping and propagating immediately on handler failure, without the executor resolving `node_id -> Component` itself, retrying, timing out, running concurrently, or depending on `ExecutionEngine`, `Component`, `CompositionGraph`, or any provider | ADR-018 (Proposed — not yet implemented; revised from an earlier single-step `StepExecutor` draft to a plan-wide `Executor`/`SequentialExecutor` at the project owner's explicit direction — see ADR-018 "Revision note"); `Executor`/`StepHandler` (`Protocol`s), `SequentialExecutor`, `ExecutionContext`, `ExecutionResult` — failures propagate unwrapped (not `ExecutionError`-wrapped), a considered departure from `Module.__call__`'s convention because the executor invokes an opaque caller-supplied handler, not a fixed `Component` contract (ADR-018 Q11) | implementation, tests, benchmark, and CI all pending — this row records the architectural decision only, per the project's standing rule that an ADR decision is not proof until implementation/tests/CI independently confirm it | planned: unit/contract/failure/adversarial tests (ADV-05..08: plan non-mutation, executor statelessness, provider independence) per ADR-018's Testing strategy section, `benchmarks/step14_execution_boundary.py` |
