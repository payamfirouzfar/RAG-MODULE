...
| A66 | An `ExecutionPlan` can be run to completion via a caller-supplied `StepHandler` (given the full `ExecutionStep`, not only `node_id`), producing a `StepExecutionResult` keyed by `node_id`, with each step's handler able to see prior steps' results through an immutable `StepExecutionContext` snapshot, stopping and propagating the exact original exception immediately on handler failure with no partial result ever produced, without the executor resolving `ExecutionStep -> Component` itself, retrying, timing out, running concurrently, or depending on `ExecutionEngine`, `Component`, `CompositionGraph`, or any provider | ADR-018 (Proposed — not yet implemented; revised three times: first from an earlier single-step `StepExecutor` draft to a plan-wide `Executor`/`SequentialExecutor` at the project owner's explicit direction; then via a 14B adversarial review that found and fixed 1 BLOCKER-tier naming collision, 1 BLOCKER-tier handler-signature gap, 1 false async-compatibility claim, and 4 further MAJOR/MINOR findings (see ADR-018 "14B adversarial review findings"); then via a 14E pre-implementation fresh audit that found a second, previously-missed naming collision — see ADR-018 "Post-merge finding"); `Executor`/`StepHandler` (`Protocol`s, synchronous-only), `SequentialExecutor`, `StepExecutionContext` (renamed from `ExecutionContext` to avoid colliding with `ragtorch.core.context.ExecutionContext`), `StepExecutionResult` (renamed from `ExecutionResult` to avoid colliding with `ragtorch.core.engine.ExecutionResult`, already exported bare in both `ragtorch.core.__all__`/`ragtorch.__all__` and pinned by `tests/unit/test_public_api.py`) — failures propagate unwrapped and un-copied (`exc_info.value is error`), a considered departure from `Module.__call__`'s convention because the executor invokes an opaque caller-supplied handler, not a fixed `Component` contract (ADR-018 Q11); context-construction is an accepted, benchmark-measured O(n²) cost in v0.1, not a silently-eaten one (ADR-018 "Context construction cost") | implementation, tests, benchmark, and CI all pending — this row records the architectural decision only, per the project's standing rule that an ADR decision is not proof until implementation/tests/CI independently confirm it | planned: CTX/EXEC/FAIL/STATE/ADV/CONTRACT/REG test matrix (25 cases) per ADR-018's Testing strategy section, `benchmarks/step14_execution_boundary.py` extended to 100,000 steps with time-per-step reporting |
