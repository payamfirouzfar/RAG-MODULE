...
| A66 | An `ExecutionPlan` can be run to completion via a caller-supplied `StepHandler` (given the full `ExecutionStep`, not only `node_id`), producing a `StepExecutionResult` keyed by `node_id`, with each step's handler able to see prior steps' results through an immutable `StepExecutionContext` snapshot, stopping and propagating the exact original exception immediately on handler failure with no partial result ever produced, without the executor resolving `ExecutionStep -> Component` itself, retrying, timing out, running concurrently, or depending on `ExecutionEngine`, `Component`, `CompositionGraph`, or any provider | ADR-018 (contract frozen via PR #12, corrected via PR #13, both post-merge-CI-confirmed on `main`); `src/ragtorch/core/execution.py` — `Executor`/`StepHandler` (`Protocol`s, synchronous-only), `SequentialExecutor`, `StepExecutionContext`, `StepExecutionResult` — failures propagate unwrapped and un-copied (`exc_info.value is error`), a considered departure from `Module.__call__`'s convention because the executor invokes an opaque caller-supplied handler, not a fixed `Component` contract (ADR-018 Q11); context-construction is an accepted, benchmark-measured O(n²) cost in v0.1, confirmed across two independent local runs (time/step grew ~780x in one run and ~560x in another from 10 to 100,000 steps — absolute wall-clock varies with machine load, but the growing-time/step shape is consistent both times — see `evaluation/step14-evaluation.md` "Performance") | none for the scoped contract; implementation, unit tests, integration tests, failure-behavior coverage, and benchmark measurements are all locally complete (358/358 tests, verified on 3.10 and 3.12, lint/format/mypy clean, `execution.py` 100% coverage); Python 3.11 and the real GitHub Actions CI run are still pending — this row's status is upgraded only after real CI confirms on the actual PR/merge SHA, per the project's standing rule that local checks are not CI | `tests/unit/core/test_execution.py` (27 tests: CTX/EXEC/FAIL/STATE/ADV/CONTRACT), `tests/integration/test_execution_boundary.py` (2 tests: dependency-aware handler chain end-to-end via the public API, mid-plan failure propagation), `benchmarks/step14_execution_boundary.py` (10/100/1,000/10,000/100,000 steps), `evaluation/step14-evaluation.md` |
