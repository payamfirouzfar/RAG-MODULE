...
| A66 | One `ExecutionStep`'s already-resolved `Component` can be invoked with one input value and an explicit `ExecutionContext`, returning the component's output or raising `ExecutionError` on failure, without the executor resolving `node_id -> Component` itself, scheduling, retrying, timing out, running concurrently, or depending on `ExecutionEngine` or any provider | ADR-018 (Proposed — not yet implemented); `StepExecutor` (`Protocol`) / `DefaultStepExecutor`, `raise ExecutionError(...) from exc` matching `Module.__call__`'s existing convention exactly (found and confirmed during the 14A audit, not assumed) | implementation, tests, benchmark, and CI all pending — this row records the architectural decision only, per the project's standing rule that an ADR decision is not proof until implementation/tests/CI independently confirm it | planned: unit/contract/failure tests per ADR-018's Testing strategy section, `benchmarks/step14_execution_boundary.py` |
